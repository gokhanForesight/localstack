...
docker-run-tests:		  ## Initializes the test environment and runs the tests in a docker container
	...
	docker run --entrypoint= -v `pwd`/tests/:/opt/code/localstack/tests/ -v `pwd`/target/:/opt/code/localstack/target/ \
		$(IMAGE_NAME_FULL) \
	    bash -c "make install-test && make init-testlibs && pip uninstall -y argparse dataclasses && DEBUG=$(DEBUG) TF_BUILD=True CIRCLE_REPOSITORY_URL=https://github.com/gokhanForesight/localstack CIRCLE_BRANCH=master LAMBDA_EXECUTOR=local PYTEST_LOGLEVEL=debug PYTEST_ARGS='$(PYTEST_ARGS)' COVERAGE_FILE='$(COVERAGE_FILE)' TEST_PATH='$(TEST_PATH)' make test-coverage"
